 ...
 * local src = [[...]]
 * local key = [[...]]
 * local result = codec.hmac_sha256_encode(src, key)
 */
static int codec_hmac_sha256_encode(lua_State *L)
{
  size_t len, klen;
  const char *cs = lua_tolstring(L, 1, &len);
  const char *key = lua_tolstring(L, 2, &klen);
...
int luaopen_codec(lua_State *L)
{
    static const struct luaL_Reg codec[] =
    {
        ...
        {"hmac_sha256_encode", codec_hmac_sha256_encode},
